...
if(!clip::where('id' , $request->clip_id)->exists())
{
    $set['error'] = true;
    $set['message'] = "Clip Not Found";
    return $this->set_json_response($set ,  $data);
}



    $clip_view =  ClipView::where('user_id',$user->id)
                    ->where('action' , 'view')
                    ->where('clip_id',$request->clip_id)->first();
if($clip_view == null)
{
$new_view =  new ClipView();
$new_view->user_id  = $user->id;
$new_view->clip_id  =$request->clip_id;
$new_view->save();

$set['error'] = false;
$set['message'] = "Added To Clip Watched Report";
return $this->set_json_response($set ,  $data);

}

$set['error'] = true;
$set['message'] = "Clip Alerady Watched";
return $this->set_json_response($set ,  $data);
...
$Wabastagan_edit->country	=  $string;
...
$set['auth'] = true;
$set['message'] = "";
$Wabastagan_edit->country = json_decode($Wabastagan_edit->country);
...
$Wabastagan->country = json_decode($Wabastagan->country);
...
$data['clip'] =  $clip ;
$data['announcement'] =  config('app.announcement');
