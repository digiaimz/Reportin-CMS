        ...
        <div class="row">
            <div class="col-md-12">
                <h4>    Create Designation
                    ...
                    <a class="btn btn-primary  " href="{{route('staff.role.view')}}">
...
margin-right: 5px;
    ...
    <form method="POST" action="{{route('create.new.designation')}}"    >
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-8 col-md-6 form-group">
            ...
            placeholder="Enter Designation Name" required="" value="{{old('name')}}"
            autocomplete="off"
            name="name" type="text"  >
            @error('name')
        ...
        <div class="col-xs-12 col-sm-8 col-md-6 form-group">
            ...
            <select  name="level"   class="  form-control" style="cursor: pointer; user-select: auto;" required>
                ...
                <option @if( old('level'  )  == "Admin") selected @endif value="Admin" style="user-select: auto;">Admin Level </option>
                <option @if( old('level'  )  == "Markazi") selected @endif value="Markazi" style="user-select: auto;">Markazi Level </option>
                <option  @if( old('level' ) == "Zone") selected @endif value="Zone" style="user-select: auto;">Zone Level </option>
                <option  @if( old('level' ) == "District") selected @endif value="District" style="user-select: auto;">District Level </option>
                <option  @if( old('level' ) == "Tehsil") selected @endif value="Tehsil" style="user-select: auto;">Tehsil Level </option>
                   </select>
            @error('level')
            <div class="alert alert-danger">@lang($message)</div>
            @enderror
        ...
        <div class="col-xs-12 col-sm-8 col-md-6 form-group">
            ...
            <option value="" style="user-select: auto;">Choose...</option>
               @foreach (\App\Forum::where('frm_shown' , 1)->orderBy('frm_ordering', 'ASC')->get() as  $forum)
                <option @if( old('forum_id'  ) ==  $forum->id_frm) selected @endif value="{{$forum->id_frm}}">{{$forum->frm_code}} - {{$forum->frm_name}}</option>
                @endforeach
...
<div class="col-xs-12 col-sm-8 col-md-6 form-group">
    <button class="btn btn-primary btn-block  " name="submit_button" type="submit" >
