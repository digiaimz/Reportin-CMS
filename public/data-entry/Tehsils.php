...
if($view == 'delete') {
	if(isset($_GET['Tid'])) {
		$sqllms  = $dblms->querylms("DELETE FROM ".TEHSILS." WHERE id_tsl = '".cleanvars($_GET['Tid'])."'");
		header('location: Tehsils.php?del=1');
	}
}
//-------------------------------------------------------
...
<div class="col-sm-12">
<div class="white-box">
<form class="form-group" style="margin-top:10px;" action="Tehsils.php" method="get">
...
<div class="col-lg-12">
<div class="white-box">
<a class="box-title m-b-0"> <a href="Tehsils.php" class="fcbtn btn btn-success btn-outline btn-1e"> <i class="fa fa-list"></i> Tehsils List </a>';
//---------------------------------------------------------------------------------
if($_SESSION['LOGINTYPE_SSS'] == 1){
//---------------------------------------------------------------------------------
echo'	
<a href="Tehsils.php?view=add" class="fcbtn btn btn-info btn-outline btn-1e"> <i class="fa fa-plus"></i> Add Tehsil</a>';
//---------------------------------------------------------------------------------
}
//---------------------------------------------------------------------------------
echo'
...
//------------------------------------------------
$sqllms  = $dblms->querylms("INSERT INTO ".TEHSILS."(
														id_dist											, 
														tsl_shown										, 
														tsl_ordering									, 
														tsl_name										, 
	   											...
	   											VALUES(
														'".cleanvars($_POST['id_dist'])."'		   		, 
														'".cleanvars($_POST['tsl_shown'])."'		   	, 
														'".cleanvars($_POST['tsl_ordering'])."'	  		,
														'".cleanvars($_POST['tsl_name'])."'	  			,
														'".cleanvars($_POST['tsl_name_ur'])."'	  		 
...
//-------------------------------------------------------
	...
	$originalImage  = $img_dir.to_seo_url(cleanvars($_POST['tsl_name'])).'_'.$officeid.".".strtolower($img[1]);
	$img_fileName   = to_seo_url(cleanvars($_POST['tsl_name'])).'_'.$officeid.".".strtolower($img[1]);
//-------------------------------------------------------
	if(in_array($extension , array('jpg','jpeg', 'gif', 'png'))) { 
//-------------------------------------------------------
		$sqllmsupload  = $dblms->querylms("UPDATE ".TEHSILS."
														SET tsl_photo = '".$img_fileName."'
												 WHERE  id_tsl		= '".cleanvars($officeid)."'");
		unset($sqllmsupload);
		$mode = '0644'; 
//-------------------------------------------------------	
...
if(!LMS_VIEW || LMS_VIEW == 'list') { 
	include("include/Pages/Tehsils/allTehsils.php");
}
//-------------------------------------------------------
if(LMS_VIEW == "add") { 
	include("include/Pages/Tehsils/addTehsils.php");
}
//-------------------------------------------------------
if(LMS_VIEW == "modify") { 
	include("include/Pages/Tehsils/modifyTehsils.php");
}
//-------------------------------------------------------
