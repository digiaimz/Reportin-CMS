...
//--------------------------------------------------------
if(($_GET['srch'])) {
	$sql2 		.= " AND ( zone_ordering LIKE '%".$_GET['srch']."%' OR 	zone_name LIKE '%".$_GET['srch']."%' OR zone_name_ur LIKE '%".$_GET['srch']."%')";
...
<form class="navbar-form navbar-left form-small" action="export.php" method="post" target="_blank">
	<input type="hidden" name="type" value="Zones">
...
<tr>
	...
	<th> Ordering </th>
    <th> Zone EN Name </th>
	<th style="text-align:right;"> Zone UR Name </th>
...
//------------------------------------------------
	$ctystatus = get_admstatus($rowstd['zone_shown']);
//------------------------------------------------
if($rowstd['zone_photo']) { 
	$stdphoto = '<img class="img-circle" height="50" width="50" src="photos/Zones/'.$rowstd['zone_photo'].'" alt="'.$rowstd['zone_name'].'"/>';
} else {
	$stdphoto = '<img class="img-circle" height="50" width="50" src="photos/Zones/default.png" alt="'.$rowstd['zone_name'].'"/>';
}
//------------------------------------------------
...
echo'
<tr>  
	<td>'.$rowstd['id_zone'].'</td>
	<td>'.$rowstd['zone_ordering'].'</td>
	<td>'.$rowstd['zone_name'].'</td>
	<td style="direction:rtl; text-align:right; font-size:25px; line-height:30px; font-family:Jameel Noori Nastaleeq, NafeesWebNaskhRegular, sans-serif;">'.$rowstd['zone_name_ur'].'</td>
...
echo'	
<td align="center">
	<a class="btn btn-xs btn-info" href="Zones.php?view=modify&Zid='.$rowstd['id_zone'].'"><i class="fa fa-pencil"></i></a>
	<!--a class="btn btn-danger btn-xs" href="Zones.php?Zid='.$rowstd['id_zone'].'&view=delete" data-popconfirm-yes="Yes" data-popconfirm-no="No" data-popconfirm-title="Are you sure want to delete?"> <i class="fa fa-trash-o" aria-hidden="true"></i></a-->
</td>';
//---------------------------------------------------------------------------------
}
//---------------------------------------------------------------------------------
...
<ul class="pagination pull-right">';
	$Nav= ""; 
if($page > 1) { 
	$Nav .= '<li><a href="Zones.php?page='.($page-1).$sqlstring.'">Prev</a></li>'; 
} 
for($i = 1 ; $i <= $NumberOfPages ; $i++) { 
if($i == $page) { 
	$Nav .= '<li class="active"><a href="">'.$i.'</a></li>'; 
} else { 
	$Nav .= '<li><a href="Zones.php?page='.$i.$sqlstring.'">'.$i.'</a></li>';
} } 
if($page < $NumberOfPages) { 
	$Nav .= '<li><a href="Zones.php?page='.($page+1).$sqlstring.'">Next</a><li>'; 
} 
	echo $Nav;
echo '
</ul>
