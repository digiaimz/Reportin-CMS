...
//--------------------------------------------------------
if(($_GET['srch'])) {
	$sql2 		.= " AND ( uni_abbr LIKE '%".$_GET['srch']."%' OR uni_name LIKE '%".$_GET['srch']."%'	)";
...
//------------------------------------------------
	$sqllms  = $dblms->querylms("SELECT *   
										FROM ".UNIVERSITIES." 
										WHERE uni_id != '' $sql2 
										ORDER BY uni_name ASC LIMIT ".($page-1)*$Limit .",$Limit");
//--------------------------------------------------
...
<tr>  
	<td>'.$srno.'</td>
	<td>'.$rowstd['uni_name'].'</td>
	<td>'.$rowstd['uni_abbr'].'</td>
	<td align="center">'.$ctystatus.'</td>
 <td align="center">
	<a class="btn btn-xs btn-info" href="Universities.php?view=modify&Uid='.$rowstd['uni_id'].'"><i class="fa fa-pencil"></i></a>
	<!--a class="btn btn-danger btn-xs" href="Universities.php?Uid='.$rowstd['uni_id'].'&view=delete" data-popconfirm-yes="Yes" data-popconfirm-no="No" data-popconfirm-title="Are you sure want to delete?"> <i class="fa fa-trash-o" aria-hidden="true"></i></a-->
</td>
</tr>';
//------------------------------------------------
} // end while loop
...
<ul class="pagination pull-right">';
	$Nav= ""; 
if($page > 1) { 
	$Nav .= '<li><a href="Universities.php?page='.($page-1).$sqlstring.'">Prev</a></li>'; 
} 
for($i = 1 ; $i <= $NumberOfPages ; $i++) { 
if($i == $page) { 
	$Nav .= '<li class="active"><a href="">'.$i.'</a></li>'; 
} else { 
	$Nav .= '<li><a href="Universities.php?page='.$i.$sqlstring.'">'.$i.'</a></li>';
} } 
if($page < $NumberOfPages) { 
	$Nav .= '<li><a href="Universities.php?page='.($page+1).$sqlstring.'">Next</a><li>'; 
} 
	echo $Nav;
echo '
</ul>
